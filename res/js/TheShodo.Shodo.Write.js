...
TheShodo.Shodo.Write = {
    // -- Settings
    videoFadeOutDuration: 6,
    videoFadeOutTiming: 2.5, // 首次加载视频播放持续时间
    alwaysSkipIntro: true, // 之前是false
    isUAInlineSVGSupported: false,
    preloadImages: [
        './res/img/write/icon_checkbox_01.png',
        './res/img/write/icon_checkbox_01_o.png',
        './res/img/write/icon_checkbox_02.png',
        './res/img/write/icon_checkbox_02_o.png',
    ].map(function(e) {
...
TheShodo.Shodo.Write.prepareStage = function(ratio, mode) {
    ...
    if (window.innerHeight >= 896) {
        ...
        // Default
        TheShodo.Shodo.Write.preloadImages.push(TheShodo.sharedBaseUrl + '/res/img/write/frame.png');
        return;
    }

    TheShodo.Shodo.Write.preloadImages.push(TheShodo.sharedBaseUrl + '/res/img/write/' + mode + '/frame.png');
...
TheShodo.Shodo.Write.showLoading = function() {
    ...
    loadingWatcher
        ...
        .register($('#top-menu img').get().filter(function(e) {
            return e.tagName != 'image'; })) // filter [SVGImageElement]
        //.register(document.getElementById('write-tools-movie'))  // mark old
...
TheShodo.Shodo.Write.playIntro = function() {
    var blocker = function(e) {
        ...
        $('<span class="playing-movie-notice" />')
            .html('<span>研墨中</span><br /><span>敬请稍等</span>')
            .css('left', (x - 162 / 2) + 'px')
            .css('top', (y - 56) + 'px')
            .appendTo($('.content'))
            .fadeIn()
...
TheShodo.Shodo.Write.initialize = function() {
    ...
    if (TheShodo.UA.isSVGSupported) {
        if ($('svg').get(0).namespaceURI == 'https://www.w3.org/TR/SVG/') {
...
//   mark old , add new 
    ...
    // show all elements & setup, mark old begin
...
TheShodo.Shodo.Write.clear = function() {
    ...
    var floatingPanel = new TheShodo.FloatingPanel.MessageBox('',
        TheShodo.Shodo.Resources.Write.String.Panel_Clear_Label || '是否清空?', [
            { label: TheShodo.Shodo.Resources.Write.String.Panel_Cancel || '取消', isCancel: true, isDefault: true }, {
                label: TheShodo.Shodo.Resources.Write.String.Panel_Delete || '确认',
...
TheShodo.Shodo.Write.onFinishButtonClicked = function(e) {
    ...
    currentBackgroundImage.crossOrigin = "*";
